    ...
    <div className="space-y-8 font-sans">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-[#1e233d] pb-6">
        <div>
          ...
          <p className="text-zinc-400 text-sm mt-1">View and manage all faculty members</p>
        </div>
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-[#1e233d] text-sm text-zinc-300">
                {teachers.map((teacher) => (
                  <tr key={teacher.id} className="hover:bg-[#16192b]/20 transition-colors">
                    <td className="p-4 font-semibold text-white">{teacher.name}</td>
                    <td className="p-4">{teacher.user?.email || 'N/A'}</td>
                    <td className="p-4">{teacher.qualification || 'Not specified'}</td>
                    <td className="p-4">{new Date(teacher.createdAt).toLocaleDateString()}</td>
                    <td className="p-4">
      ...
      <div className="bg-[#16192b]/50 border border-[#1e233d] rounded-xl p-6">
        <h3 className="text-sm font-bold text-white mb-2">How to Add New Teachers</h3>
        <ol className="text-xs text-zinc-400 space-y-1 list-decimal list-inside">
          <li>Create a user in Supabase Dashboard Authentication section</li>
          <li>Set user metadata role to TEACHER</li>
          <li>User logs in - their teacher profile is created automatically</li>
          <li>Use the API to update their qualification and class assignments</li>
        </ol>
      </div>
    </div>
  );
